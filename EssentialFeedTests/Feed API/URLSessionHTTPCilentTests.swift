//
//  URLSessionHTTPCilentTests.swift
//  EssentialFeedTests
//
//  Created by Minhaz on 04/09/22.
//

import XCTest
import EssentialFeed

class URLSessionHTTPCilentTests: XCTestCase {
    ...
    func test_getFromURL_performGETRequestWithURL() {
        ...
        URLProtocolSub.observeRequests { request in
    ...
    private class URLProtocolSub: URLProtocol {
        ...
        override func startLoading() {
            if let data = URLProtocolSub.stub?.data {
                client?.urlProtocol(self, didLoad: data)
            }
            
            if let response = URLProtocolSub.stub?.response {
                client?.urlProtocol(self, didReceive: response, cacheStoragePolicy: .notAllowed)
            }
            
            if let error = URLProtocolSub.stub?.error {
